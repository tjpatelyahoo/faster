    ...
    try:
        for i in range(arg-1, len(links)):
            ...
            if "acecwply" in url:
                ...
                api_url_call = f"https://head-micheline-botupdatevip-f1804c58.koyeb.app/get_keys?url={url}@botupdatevip4u&user_id=741668895&token=741Pp484kdyuT7vFWqr"
                #api_url_call = f"https://shefu-api-final.vercel.app/shefu?url={url}@crystalofficial2&user_id={user_id}"
                #api_url_call = f"https://shefu-api-final.vercel.app/shefu?url={url_norm}@ITSGOLU_FORCE&user_id=6243077977"
...
async def text_handler(bot: Client, m: Message):
    ...
    link_lines = [l for l in lines if "://" in l]

    if not link_lines:
        await m.reply_text("Invalid link format.")
        return

    # Parse like TXT handler
    # Parse like TXT handler (FIXED)
# Parse like TXT handler (FIXED)
    links = []
    ...
    for line in link_lines:
        ...
        name, url = line.split(":", 1)   # FIRST ':' only
    ...
    if raw_text2 not in {"144", "240", "360", "480", "720", "1080"}:
        raw_text2 = "480"

    # ===============================
    # PROCESS LINKS (TXT-LIKE FLOW)
    # ===============================

    count = 1
    failed_count = 0
    channel_id = m.chat.id
    b_name = "Direct Upload"

    for name, url in links:
        try:
            show = (
                f"<i><b>📥 Fast Video Downloading</b></i>\n"
                f"<blockquote><b>{str(count).zfill(3)}) {name}</b></blockquote>"
            )
            ...
            # Basic DRM / non-DRM split (safe default)
            if "drm" in url or "mpd" in url:
...
async def details_callback(client, callback_query: CallbackQuery):
    ...
    details_text = (
        "**📋 Bot Details 📋**\n\n"
        "• 🤖 Bot Name: DRM BY ITSGOLU & ANYBODY 🦋\n"
        "• 👨‍💻 Developer: IT'sGOLU.™® & ANYBODY\n"
